...
file_path_vars <- "data/Hof_luetke_input_table.xlsx"
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(width = 8,
              ...
              tags$h6(
                "This app simulates the present value of converting a treeless arable field into a fruit and honey agroforestry system, based on an agroforestry plan developed by Ms. Frauke Ganswind of Arbeitsgemeinschaft bäuerliche Landwirtschaft - Landesverband Nordrhein-Westfalen e.V. in Germany.",
                tags$br(),
                tags$br(),
              ...
              div(class = "scroll-xy",
                  plotOutput("plot4_ui", height = "700px"),
                  ),
              br(),
              ...
              div(class = "scroll-xy",
                  plotOutput("plot5_ui", height = "700px"),
                  ),
              br(),
              ...
              # div(class = "scroll-xy",
              # plotOutput("plot6_ui", height = "700px"),
              # ),
              # br(),
              ...
              # 
              # div(class = "scroll-xy",
              # plotOutput("plot7_ui", height = "700px"),
              # ), 
              # br(),
              ...
              div(class = "scroll-xy",
                  plotOutput("plot8_ui", height = "700px"),
              ),
              br(),
...
server <- function(input, output, session) {
  ...
  get_base_dir <- function() {
    if (Sys.info()[["sysname"]] == "Windows")
      "user-states/Germany2"
    else
      "/srv/shiny-app-data/user-states/Germany2"
  }
  ...
  observeEvent(mcSimulation_results(), {
    ...
    plot1 <-
      decisionSupport::plot_distributions(mcSimulation_object = mc_data,
                                          ...
                                          new_names = c("Monoculture (baseline)", "Agroforestry intervention"),
                                          x_axis_name = "NPV (€)",
      ...
      add_meta(
        ...
        caption  = "Figure 1 shows the comparison of Net Present Value (NPV) outcomes for agroforestry (fruit trees with honey) vs monoculture system (baseline). The x-axis displays NPV values (i.e., the sum of discounted annual cash flows) and y-axis displays the probability of each NPV amount to occur (i.e., higer y-values indicate higher probability)." 
        #The higher and wider the box, the greater the potential return and variability in outcomes under that system. )
      )
    plot2 <- decisionSupport::plot_distributions(
      mc_data, "NPVtrade_off",
      method     = "smooth_simple_overlay",
    ...
    plot3 <- decisionSupport::plot_distributions(
      mc_data,
      vars      = c("NPV_decis_nofund", "NPVtrade_off", "NPV_decis_DeFAF"),
      method    = "boxplot",
      old_names = c("NPV_decis_nofund", "NPVtrade_off", "NPV_decis_DeFAF"),
...
Scenarios involving funding (like DeFAF-suggested or EcoScheme3 and regional) generally show higher NPV ranges than the No funding, however it not necessarily better suggesting the current financial support is insufficient to sustain agroforestry.'
    ...
    observeEvent(input$confirm_evpi, {
      ...
      tryCatch({
        ...
        plot8 <- plot_evpi(evpi_result, decision_vars = "NPVtrade_off",
                           new_names = "") +
