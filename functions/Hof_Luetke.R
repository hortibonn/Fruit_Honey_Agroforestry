...
AF_benefit <- function(x, varnames) {
  ...
  AF_raspberry_yield[time_to_first_raspberry_c] <- raspberry_yield_max_p * Raspberry_row_legth * (percentage_raspberry_first_yield_p/100)
 ...
 # Alley cropping system cost
  ...
  AF_clover_labour_cost[Clover_indices] <- vv(clover_ley_labour_p, var_CV = var_cv_p, length(Clover_indices))*Arable_area_AF*(Labour_costs[Clover_indices]*af_labour_increase_p/100)
  AF_org_wheat_labour_cost[Org_wheat_indices] <- vv(org_wheat_labour_p, var_CV = var_cv_p, length(Org_wheat_indices))*Arable_area_AF*(Labour_costs[Org_wheat_indices]*af_labour_increase_p/100)
  AF_buckwheat_labour_cost[Buckwheat_indices] <- vv(buckwheat_labour_p, var_CV = var_cv_p, length(Buckwheat_indices))*Arable_area_AF*(Labour_costs[Buckwheat_indices]*af_labour_increase_p/100)
  AF_flax_labour_cost[Flax_indices] <- vv(rye_labour_p, var_CV = var_cv_p, length(Rye_indices))*Arable_area_AF*(Labour_costs[Rye_indices]*af_labour_increase_p/100)
  AF_rye_labour_cost[Rye_indices] <- vv(flax_labour_p, var_CV = var_cv_p, length(Flax_indices))*Arable_area_AF*(Labour_costs[Flax_indices]*af_labour_increase_p/100)
  AF_org_soy_labour_cost[Org_soy_indices] <- vv(org_soy_labour_p, var_CV = var_cv_p, length(Org_soy_indices))*Arable_area_AF*(Labour_costs[Org_soy_indices]*af_labour_increase_p/100)
  AF_spelt_labour_cost[Spelt_indices] <- vv(spelt_labour_p, var_CV = var_cv_p, length(Spelt_indices))*Arable_area_AF*(Labour_costs[Spelt_indices]*af_labour_increase_p/100)
  ...
  #AF no fund 
  AF_total_cost_nofund <- AF_total_cost - ES3_application
  AF_bottom_line_benefit_nofund <- (AF_total_benefit_nofund - AF_total_cost_nofund)
  
  #Calculating NPVs and Cash Flows####
  #AF System no fund
  AF_NPV_nofund <- discount(AF_bottom_line_benefit_nofund, discount_rate=discount_rate_p,
                            calculate_NPV = TRUE)#NVP of AF system
  AF_cash_flow_nofund <- discount(AF_bottom_line_benefit_nofund,discount_rate=discount_rate_p,
                                  calculate_NPV = FALSE)#Cash flow of AF system
  AF_cum_cash_flow_nofund <- cumsum(AF_cash_flow_nofund) #Cumulative cash flow of AF system
  ...
  Tradeoff_benefit_nofund <- AF_bottom_line_benefit_nofund - Treeless_bottom_line_benefit
  #Tradeoff_benefit_farm <- AF_bottom_line_benefit_farm - Treeless_bottom_line_benefit
  
  NPV_tradeoff_nofund <- discount(Tradeoff_benefit_nofund, discount_rate = discount_rate_p,
                                  calculate_NPV = TRUE )
  
  # Scenario "DeFAF-Subsidy" ####
  ...
  DeFAF_ES3 <- annual_funding_schemes_c - ES3_application #suggested level of ES3 subsidy
